...
def load_detector_parameters(file_path):
    with open(file_path, 'r') as file:
        data = yaml.load(file, Loader=yaml.SafeLoader)
...
def main():
    ...
    # Load board data
    board_corners, board_ids = load_board_data(BOARD_DATA_DIR /
                                               'board_layout.yaml')

    # Load camera parameters
    camera_matrix, distortion_coeffs = load_camera_parameters(
    ...
    detector_params = load_detector_parameters(BOARD_DATA_DIR /
    ...
    while True:
        ...
        # Draw the detected markers and rejected candidates
        image_markers = cv2.aruco.drawDetectedMarkers(frame.copy(), corners,
                                                      ids)
