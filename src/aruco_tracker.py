...
class ArucoSingleTracker():
    ...
    def __init__(self,
        ...
        self._cap = cv2.VideoCapture(video_device)
    ...
    def track(self, loop=True, verbose=False, show_video=None):
        ...
        while not self._kill:
            ...
            if not ids is None and self.id_to_find in ids[0]:
                ...
                aruco.drawAxis(frame, self._camera_matrix,
                ...
                if show_video:
                    ...
                    cv2.putText(frame, str_position, (0, 100), self.font, 1,
                    ...
                    cv2.putText(frame, str_attitude, (0, 150), self.font, 1,
                                (0, 255, 0), 2, cv2.LINE_AA)

                    str_position = "CAMERA Position x=%4.0f  y=%4.0f  z=%4.0f" % (
                        pos_camera[0], pos_camera[1], pos_camera[2])
                    cv2.putText(frame, str_position, (0, 200), self.font, 1,
                                (0, 255, 0), 2, cv2.LINE_AA)

                    # Get the attitude of the camera respect to the frame
                    roll_camera, pitch_camera, yaw_camera = self._rotation_matrix_to_euler_angles(
                        self._R_flip * R_tc)
                    str_attitude = "CAMERA Attitude r=%4.0f  p=%4.0f  y=%4.0f" % (
                        math.degrees(roll_camera), math.degrees(pitch_camera),
                        math.degrees(yaw_camera))
                    cv2.putText(frame, str_attitude, (0, 250), self.font, 1,
                                (0, 255, 0), 2, cv2.LINE_AA)
